...
public class TutorialManager : MonoBehaviour
{
    ...
    void Update()
    {
        if (10.0f <= timeManager.GetComponent<TimeManager>().getTimeCurrent())
        {
            tutorial.text = "라이터에 다가가 F 버튼을 누르면 획득할 수 있습니다 \n G 버튼을 눌러 라이터를 켜고 끌 수 있습니다";
        }
        if (20.0f <= timeManager.GetComponent<TimeManager>().getTimeCurrent())
        {
            tutorial.text = "촛대에 다가가 E 버튼을 누르면 불을 붙일 수 있습니다. \n 몬스터를 피해 10개의 촛불을 모두 켜면 탈출구가 열립니다 행운을 빕니다";
        }
        if (30.0f <= timeManager.GetComponent<TimeManager>().getTimeCurrent())
        {
            tutorial.text = "";
        }
    }
